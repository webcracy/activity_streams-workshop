module Aidentiti
  class Base
    ...
    def grab_credentials
      perform_get('/api/grab_credentials.json')
    end
    
    # Aidentiti
    def grab_relationships
      perform_get('/api/grab_relationships.json')
    end
    
    def vocabulary(id)
      perform_get('/api/vocabulary.json', :query => {:id => id})
    end
    
    def vocabularies
      perform_get('/api/vocabularies.json')
    end
    
    # Aidentiti
    def grab_hierarchy
      perform_get('/api/grab_hierarchy.json')
    end
    
    # Aidentiti
    def post_activity(activity)
      perform_post('/api/activity.json', {:body => activity.to_json,:headers => {'Content-Type' => 'application/json', 'Accept' => 'application/json'}})
    end
    
    # Aidentiti
    def test_activity(activity)
      perform_post('/api/activity_test.json', {:body => activity.to_json,:headers => {'Content-Type' => 'application/json', 'Accept' => 'application/json'}})
    end
    ...
    private
      def perform_get(path, options={})
        Aidentiti::Request.get(self, path, options)
      end
      
      def perform_post(path, options={})
        Aidentiti::Request.post(self, path, options)
      end
  end
end